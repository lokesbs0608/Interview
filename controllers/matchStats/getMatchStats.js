  ...
  try {
    ...
    const matchExtras = await DeliveriesUpdateTable.findAll({
      attributes: [
        ...
        [Sequelize.fn("SUM", Sequelize.col("isWide")), "wides"],
        [Sequelize.fn("SUM", Sequelize.col("LegByes")), "leg_byes"],
        [Sequelize.fn("SUM", Sequelize.col("Byes")), "byes"],

        [
          Sequelize.fn(
            "COUNT",
            Sequelize.literal(
              "DISTINCT CASE WHEN isNoBall IS NOT NULL AND isNoBall != '' THEN CONCAT(matchId, batsman_runs, batsman, bowler) ELSE NULL END"
            )
          ),
          "no_balls",
        ],
    ...
    const lowestRunScorer = batsmanStats.reduce((prev, current) =>
      prev.total_runs < current.total_runs ? prev : current
    );
    ...
    const bestEconomyBowler = bowlerStats.reduce((prev, current) => {
    ...
    const statistics = {
      ...
      best_economy_bowler: bestEconomyBowler,
    };
